...
namespace {
  struct AntiHack: ModulePass {
    ...
    AntiHack(): ModulePass(ID), configJson(0) {};
  ...
  bool AntiHack::runOnModule(Module& m)
  {
    bool modified = false;
    ...
    : ModulePass(ID), configJson(0)
  {
    ...
    std::error_code EC; 
    logFile = new llvm::raw_fd_ostream(logPath, EC, sys::fs::F_Append);
    if (EC) {
