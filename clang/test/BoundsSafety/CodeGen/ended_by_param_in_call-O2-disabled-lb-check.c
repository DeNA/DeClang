...
// RUN: %clang_cc1 -O2 -triple arm64-apple-iphoneos -x objective-c  -emit-llvm -fexperimental-bounds-safety-objc -fbounds-safety -fno-bounds-safety-bringup-missing-checks=ended_by_lower_bound -o - %s | FileCheck %s
#include <ptrcheck.h>
...
// CHECK-NEXT:  [[ENTRY:.*:]]
// CHECK-NEXT:    [[CMP26_NOT:%.*]] = icmp ugt ptr [[START]], [[END]], !annotation [[META2]]
...
// CHECK-NEXT:  [[ENTRY:.*:]]
// CHECK-NEXT:    [[CMP28_NOT:%.*]] = icmp slt i32 [[COUNT]], 0, !annotation [[META2]]
// CHECK-NEXT:    br i1 [[CMP28_NOT]], label %[[TRAP:.*]], label %[[CONT:.*]], !annotation [[META2]]
// CHECK:       [[TRAP]]:
// CHECK-NEXT:    tail call void @llvm.ubsantrap(i8 25) #[[ATTR5]], !annotation [[META2]]
// CHECK-NEXT:    unreachable, !annotation [[META2]]
// CHECK:       [[CONT]]:
